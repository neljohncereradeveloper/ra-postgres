...
export const ReprintBallotHalfLetter: React.FC<{
  castVote: CastVoteReprint;
  onExit?: () => void;
}> = ({ castVote, onExit }) => {
  ...
  const hasSelections = castVote.groupCandidates.some(
      ...
      >
        ...
        <CardContent className="bg-white print:bg-white overflow-auto">
          ...
          <div className="flex flex-col gap-3 items-center">
            {castVote.groupCandidates.map((group) => (
                ...
                <ul className="list-disc text-xs py-1 flex flex-col gap-1 items-center">
                  {group.candidates.length > 0 ? (
                    group.candidates.map((candidate) => (
                      <li key={candidate.id} className="font-medium text-black">
                        {candidate.name}
                      </li>
                    ))
                  ) : (
          ...
          <h2 className="text-xs text-black font-bold tracking-wide uppercase text-center mb-5">
            --- Reprinted ---
          </h2>
