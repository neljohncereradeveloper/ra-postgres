...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${API_BASE_URL}/candidates?term=${encodeURIComponent(
      term
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: `Failed to fetch candidates: ${response.statusText}` },
...
// POST handler for creating candidates
      ...
      return NextResponse.json(
        {
          error:
            errorData?.message ||
            `Failed to create candidate: ${response.statusText}`,
        },
        { status: response.status }
      );
    }
