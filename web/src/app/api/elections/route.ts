...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${API_BASE_URL}/elections?term=${encodeURIComponent(
      term
    ...
    const response = await fetch(url, {
      ...
      next: { revalidate: 0 }, // Prelection caching
    });
...
// POST handler for creating elections
      ...
      return NextResponse.json(
        {
          error:
            errorData?.message ||
            `Failed to create election: ${response.statusText}`,
        },
        { status: response.status }
      );
    }
